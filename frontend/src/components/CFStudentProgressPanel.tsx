  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', gap: '1rem', color: 'var(--text-muted)' }}>
        ...
        <span>Cargando cursos de CourseFactory...</span>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div>
        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-main)' }}>
          Dashboard de Seguimiento de CourseFactory (Iframe Engine)
        </h3>
        <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          Análisis de uso, retenido y avance de alumnos directamente en los reproductores e iframes embebidos de CourseFactory.
        </p>
      </div>
      ...
      {loadingProgress ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '200px', gap: '0.75rem', color: 'var(--text-muted)' }}>
          <Loader2 size={24} className="animate-spin text-primary" />
          <span>Obteniendo avance del curso desde el iframe de CourseFactory...</span>
        </div>
      ) : error ? (
        <div style={{ padding: '1.5rem', background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', borderRadius: '8px', fontSize: '0.9rem' }}>
          {error}
        </div>
      ) : students.length === 0 ? (
        <div style={{ padding: '2rem', textAlign: 'center', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '12px', color: 'var(--text-muted)' }}>
          No hay alumnos registrados con actividad dentro del reproductor de CourseFactory para este curso.
        </div>
      ) : (
        <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ background: 'var(--bg-primary)', borderBottom: '1px solid var(--border)', color: 'var(--text-muted)', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.5px' }}>
                  ...
                  <th
                    ...
                    style={{ padding: '0.75rem 1rem', width: '200px', cursor: 'pointer', userSelect: 'none', color: sortField === 'progress' ? 'var(--primary)' : 'inherit' }}
                  >
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                ...
                {sortedStudents.map(student => {
                  ...
                  return (
                    <React.Fragment key={student.alumnoId}>
                      <tr
                        style={{ borderBottom: '1px solid var(--border)', background: isExpanded ? 'var(--bg-primary)' : 'transparent', transition: 'background 0.2s' }}
                      >
                        <td style={{ padding: '0.75rem 1rem', textDecoration: 'none' }}>
                          <button
                        ...
                        <td style={{ padding: '0.75rem 1rem' }}>
                          <div style={{ fontWeight: 700, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            ...
                            {student.redeemedCode && (
                              <span title={`Código canjeado: ${student.redeemedCode}`} style={{ background: '#ecfdf5', color: '#059669', border: '1px solid #a7f3d0', fontSize: '0.65rem', padding: '2px 6px', borderRadius: '4px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
                                <Key size={10} /> {student.redeemedCode}
                              </span>
                            )}
                          </div>

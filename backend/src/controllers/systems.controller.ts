...
4. Genera un encabezado de Módulo destacado al inicio con el título "${moduleName}" usando la fuente de títulos '${template.design?.headlineFont}' y el color principal ${template.design?.primaryColor}.
   - **Clase CSS del Encabezado**: El contenedor del encabezado del módulo debe tener obligatoriamente la clase CSS \`module-header\`. Debe tener un padding de 2rem (o similar) y un margen de 0, extendiéndose hasta los bordes laterales y el borde superior (sin márgenes externos) para ocupar todo el ancho superior de forma continua.
   - **Clase nolink (CRÍTICO)**: El título principal (elemento \`h1\`) y cualquier contenedor de texto del encabezado deben llevar la clase CSS \`nolink\` y estar envueltos en un \`<span class="nolink">\` (ej: \`<h1 class="bebas-title nolink"><span class="nolink">${moduleName}</span></h1>\`). Esto evita de raíz que Moodle convierta el título en un enlace.
   - **Regla CSS de Respaldo**: Por seguridad, añade al bloque \`<style>\` al inicio del HTML la siguiente regla CSS:
     \`.module-header a, .module-header a:hover, .module-header a:visited, .module-header a:active, #region-main .module-header a { color: inherit !important; text-decoration: none !important; }\`
5. **CONTENEDOR DE CONTENIDOS (OBLIGATORIO)**: Todo el contenido restante de la clase (los bloques de clases, texto, videos, etc.) que viene después del encabezado del módulo debe estar envuelto en un contenedor secundario:
